...
const displayLikedPosts = () => {
  ...
  likedItems.textContent = "";
  const likedPosts = getLikedPosts();
  likedPosts.forEach((post) => {
    const div = createPost(post);
    document.getElementById("liked-items").appendChild(div);
  });
};

const displayReportedPosts = () => {
  const reportedItems = document.getElementById("reported-items");
  reportedItems.textContent = "";
  const reportedPosts = getReportedPosts();
  reportedPosts.forEach((post) => {
    const div = createPost(post);
    document.getElementById("reported-items").appendChild(div);
  });
};
